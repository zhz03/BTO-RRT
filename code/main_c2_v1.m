...
% 
plot(Pinv(:,2),Pinv(:,1),'-r','LineWidth',2)
% P1 = round(Pinv);
P_greed = optimization(map,Pinv);
P_greed  = [P_greed;q_goal];
plot(P_greed(:,2),P_greed(:,1),'-y','LineWidth',2)
...
figure(3)
imshow(map); hold on;
smooth = spcrv([[P_greed(1,2) P_greed(:,2)' P_greed(end,2)];[P_greed(1,1) P_greed(:,1)' P_greed(end,1)]],3); 
plot(smooth(1,:),smooth(2,:), '-g','LineWidth',2); 
