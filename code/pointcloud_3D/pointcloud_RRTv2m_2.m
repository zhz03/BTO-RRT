...
q_goal = [30,100,90,1,0,0];
q_init = [230,250,90,0,0,0];

% q_init = [70,215,100,0,0,0];
% q_goal = [35,20,100,1,0,0];
%hot
% 1st
%q_goal = [120,190,30,1,0,0];
%q_init = [20,20,30,0,0,0];
%2nd
q_init = [20,120,22,0,0,0];
q_goal = [190,130,25,1,0,0];
% test
...
hold on
plot3(path(:,1),path(:,2),path(:,3),'y','LineWidth',2);  
hold off

% downsample
path_opt = downsample(path,ptCloud,sizemax);
%up sample
if (~isempty(path_opt))
...
% keypoint smoother
path_up = P';
keypoints = downsample(path_up,ptCloud,sizemax);
%keypoints = downsample(keypoints1,ptCloud,sizemax);
kp = keypoints';
cs_xyz = cscvn(kp(:,[1:end]));
...
%plot3(keypoints(:,1),keypoints(:,2),keypoints(:,3),'*r','LineWidth',4);
hold on
fnplt(cs_xyz,'-b',2)
...
%% function 5: downsampel function
function  path_opt = downsample(path,ptCloud,sizemax)
